...
class OpentronsEdgeDevice:
    def __init__(self, broker_host: str = "192.168.50.131", broker_port: int = 8883, 
        ...
        self.execution_log_topic = f"lab/{self.device_id}/execution_log"  # Send real-time execution logs
        ...
        # Execution logs storage for HTTP API
        self.execution_logs = []
        self.max_logs = 100
    ...
    def handle_protocol_message(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            # Execute protocol in background thread with real-time monitoring
            import threading
            thread = threading.Thread(
                target=self.execute_protocol_with_monitoring,
...
'''
    ...
    def execute_protocol_with_monitoring(self, protocol_id: str, protocol_data: Dict[str, Any]):
        """Execute protocol with real-time monitoring like Jupyter notebook"""
        try:
            # Extract protocol code
            protocol_code = protocol_data.get('protocol_code')
            if not protocol_code:
            ...
            logger.info(f"Starting protocol execution with real-time monitoring on robot {self.robot_ip}")
            ...
            # Send initial execution log
            self.send_execution_log(protocol_id, {
                "type": "protocol_started",
                "message": f"Protocol execution started",
                "run_id": run_id,
                "protocol_id": protocol_id_robot,
                "timestamp": datetime.now().isoformat()
            })
            
            # Monitor run status with real-time updates (like Jupyter notebook)
            timeout_seconds = protocol_data.get('timeout_seconds', 300)
            execution_result = self.monitor_run_with_realtime_updates(protocol_id, run_id, timeout_seconds)
            ...
            logger.error(f"Error in protocol execution with monitoring: {e}")
            self.send_protocol_failed(protocol_id, str(e))
        finally:
            # Update device status
            self.processing = False
    ...
    def monitor_run_with_realtime_updates(self, protocol_id: str, run_id: str, timeout_seconds: int = 300) -> Dict[str, Any]:
        """Monitor run status with real-time updates like Jupyter notebook"""
        try:
            status_url = f"{self.api_base_url}/runs/{run_id}"
            commands_url = f"{self.api_base_url}/runs/{run_id}/commands"
            poll_interval = 5  # Check every 5 seconds (same as Jupyter notebook)
            elapsed_time = 0
            
            # Track seen commands (like Jupyter notebook)
            seen_command_ids = set()
            last_dispense_cmd = None
            
            while elapsed_time < timeout_seconds:
                try:
                    # --- Get run status (like Jupyter notebook) ---
                    run_resp = requests.get(status_url, headers=self.headers, timeout=10)
                    if not run_resp.ok:
                        logger.warning(f"Failed to get run status: {run_resp.status_code}")
                        time.sleep(poll_interval)
                        elapsed_time += poll_interval
                        continue
                    ...
                    self.send_execution_log(protocol_id, {
                        "type": "status_update",
                        "message": f"Current run status: {status}",
                        "status": status,
                        "run_id": run_id,
                        "elapsed_time": elapsed_time,
                        "timestamp": datetime.now().isoformat()
                    ...
                    logger.info(f"Current run status: {status} (elapsed: {elapsed_time}s)")
                    
                    # --- Get new commands (like Jupyter notebook) ---
                    cmd_resp = requests.get(commands_url, headers=self.headers, timeout=10)
                    if cmd_resp.ok:
                        for cmd in cmd_resp.json()["data"]:
                            ...
                            if ctype in ("aspirate", "dispense") and volume is not None:
                                command_message = f"🔄 {ctype.capitalize()} Volume: {volume} µL | Location: {location}"
                                
                                # Send command update via MQTT
                                self.send_execution_log(protocol_id, {
                    ...
                    # --- Exit when run finishes (like Jupyter notebook) ---
                    if status in ["succeeded", "failed", "stopped"]:
                        completion_message = f"✅ Protocol {status}."
                        ...
                        self.send_execution_log(protocol_id, {
                            ...
                            "run_id": run_id,
                            "elapsed_time": elapsed_time,
                            "last_dispense_cmd": last_dispense_cmd,
                            "timestamp": datetime.now().isoformat()
                        })
                        ...
                        return {
                            ...
                            "elapsed_time": elapsed_time,
                            "last_dispense_cmd": last_dispense_cmd,
                            "seen_commands": len(seen_command_ids)
                        }
                    ...
                    logger.warning(f"Error during monitoring iteration: {e}")
                    time.sleep(poll_interval)
                    elapsed_time += poll_interval
            
            # Timeout reached
            timeout_message = f"Run monitoring timed out after {timeout_seconds} seconds"
            self.send_execution_log(protocol_id, {
                "type": "timeout",
            ...
            logger.error(f"❌ Error monitoring run with real-time updates: {e}")
            return {
                "status": "error",
                "run_id": run_id,
                "error": str(e)
    ...
    def send_execution_log(self, protocol_id: str, log_data: Dict[str, Any]):
        """Send real-time execution log via MQTT and store locally"""
        try:
            message = {
                'protocol_id': protocol_id,
                'device_id': self.device_id,
            ...
            # Send via MQTT
            self.client.publish(self.execution_log_topic, json.dumps(message), qos=1)
            logger.debug(f"Sent execution log: {log_data.get('type', 'unknown')}")
            
            # Also store locally for HTTP API access
            self.add_execution_log(log_data)
    ...
    def setup_flask_routes(self):
        ...
        @self.app.route('/api/execution_logs', methods=['GET'])
        def get_execution_logs():
            """Get execution logs via HTTP API"""
            try:
                return jsonify({
                    'success': True,
        ...
        def get_device_status():
            """Get device status via HTTP API"""
            try:
                return jsonify({
                    'success': True,
    ...
    def run_flask_server(self):
        ...
        try:
            # Use port 5000 for the edge device HTTP API
            self.app.run(host='0.0.0.0', port=5000, debug=False, use_reloader=False)
        except Exception as e:
            logger.error(f"Failed to start Flask server: {e}")
    
    def add_execution_log(self, log_data: Dict[str, Any]):
        """Add execution log to local storage"""
        try:
            ...
            logger.debug(f"Added execution log: {log_data.get('type', 'unknown')}")
...
def main():
    ...
    parser.add_argument('--broker', default='bearslab.tail74475c.ts.net', help='MQTT broker hostname (default: bearslab.tail74475c.ts.net)')
