...
class GrblHALController(SerialController):
    """
    ...
    SAFE_MOVE_HEIGHT = -5 # safe height for Z axis in mm
    ...
    def __init__(
        ...
        self._current_position = Position(x=0.0, y=0.0, z=0.0)
        self._feed: int = feed
    ...
    def get_axis_limits(
        ...
        Args:
            axis: Optional axis name ('X', 'Y', 'Z'). If None, returns all limits.
    ...
    def home(self, axis: Optional[str] = None) -> None:
        ...
        Args:
            axis: Optional axis to home ('X', 'Y', 'Z').
    ...
    def move_absolute(
        ...
        Examples:
            ...
            >>> controller.move_absolute(position=pos)  # z uses current value
        """
        ...
        self._logger.info(
            "Preparing absolute move to X:%s, Y:%s, Z:%s at F:%s",
            position.x,
            position.y,
            position.z,
            feed_rate,
    ...
    def move_relative(
        ...
        abs_position = Position(x=abs_x, y=abs_y, z=abs_z)
        ...
        feed_rate = feed if feed is not None else self._feed
        self._logger.info(
            "Preparing relative move by dX:%s, dY:%s, dZ:%s at F:%s",
    ...
    def _execute_move(
        ...
        Safe move pattern:
        1. If X or Y movement is needed, first move Z to safe height
        2. Then move X, Y to target
        3. Finally move Z to target

        Args:
            position: Position with absolute positions for X, Y, Z axes
    ...
    async def get_position(self) -> Position:
        ...
        Returns:
            Position containing X, Y, and Z positions
